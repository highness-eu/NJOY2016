...
contains
   ...
   !    slim    logarithm of minim S(alpha, beta) (def=-225)
   !
   ...
   !                   5  lead
   !                   6  iron
   !                   98  NCrystal-LEAPR (coherent or incoherent)
   !                   99  NCrystal-LEAPR (coherent or incoherent) scaled
   !                   100 mixed elastic with NCrystal-LEAPR
   ...
   !
   !    --- NCrystal (iel >=98)  ---
   ! card 6b - NCrystal command. For example:
   !    "V_sg229.ncmat"/
   !
   ...
   !--------------------------------------------------------------------!
   ! Here are the additional/changed parameters for iel >=98            !
   ! implementation.                                                    !
   !--------------------------------------------------------------------!
   ...
   real(kr),dimension(:),allocatable::current_temp
   ...
   smin=1.0e-75_kr
   slim=-225.e0_kr
   read(nsysi,*) mat,za,isabt,ilog,smin,slim
   write(nsyso,'(/&
     ...
     &  '' smin.................................. '',es10.3/&
     &  '' slim.................................. '',f10.3)')&
     &  ntempr,iprint,nphon,mat,nint(za),isabt,ilog,smin,slim
   ...
   if (iel.ge.98) write(nsyso,'(/&
     &''*** Warning.  Free atom cross section will be '',&
     &''overwritten with value from NCrystal ***'')')
   if (iel.ge.98) call mess('leapr',' Warning.  Free atom cross section will be ',&
     'overwritten with value from NCrystal.')

   if (iel.eq.100) write(nsyso,'(/&
     &''*** Warning.  Mixed elastic (iel=100) ENDF files '',&
     &''are not standard files ***'')')
   if (iel.eq.100) call mess('leapr',' Mixed elastic (iel=100) ENDF files',&
     'are not standard files.')
   ...
   if (iel.ge.98.and.mss.gt.0) write(nsyso,'(/&
     &''*** Warning. For iel.ge.98 secondary atom '',&
     &''information is ignored ***'')')
   if (iel.ge.98.and.mss.gt.0) call mess('leapr',' Warning. For iel.gt.98 secondary atom ',&
     'information is ignored.')

   !--------------------------------------------------------------------!
   ! This is where NCrystal command is read in and the arrays are       !
   ! allocated. For iel<98 "nedge" ,which is a number of bragg edges,   !
   ! is an array with 1 value, because nedge before was an integer.     !
   ! We also use a separate variable ntemp_elastic to keep the number   !
   ! of temperatures needed for iel >= 98 different than ntempr for     !
   ! clarity. Coherent or incoherent flag is used to determine what type!
   ! of the scatterer the element is, and hence for iel=98 and iel=99   !
   ! calculate the more relevant for cross section. For iel=100         !
   ! (mixed elastic implementation) calculate  both cross               !
   ! sections hence the flag is set to 0, as well as for the iel < 98.  !
   ! Flag coherent_or_incoherent will have value 1 for coherent and     !
   ! value 2 for incoherent scatterer. The value will be determined     !
   ! based on the ratio between coherent and incoherent bound cross     !
   ! sections that are passed back from ncrystal_wrapper,               !
   ! Bound incoherent and coherent xs are also passed back from the     !
   ! wrapper for iel=100.                                               !
   !                                                                    !
   ! We also increased the maximum number of bragg edges (maxb) because !
   ! for some materials NCrystal calculates a large number of edges.    !
   ! Because ENDF format requires that all temperatures have same number!
   ! of bragg edges, for the temperatures (usually temperatures that are!
   ! are greater than the starting temperature), where the number of    !
   ! of bragg edges passed back in is less than for the first           !
   ! temperature we get the value of last bragg edge and store it in    !
   ! last_edge. Then this value will be later used to pad the bragg     !
   ! array until the sizes match.                                       !
   !                                                                    !
   ! Also, in order for NCrystal to know with what atom it is dealing,  !
   ! in case of multi-atomic molecules, we send to ncrystal_wrapper the !
   ! values for Z and A of the element, so it is essential that the     !
   ! users follow originally proposed LEAPR convention for za in card 4:!
   ! **za      1000*z+a for principal scatterer**                       !
   ! When za is defined this way, NCrystal can handle isotopes as well, !
   ! according to Z and A that get calculated. In order to keep         !
   ! consistent with NCrystal we also pass back the free atom XS (scr)  !
   ! from NCrystal.                                                     !
   !--------------------------------------------------------------------!
   if (iel.gt.0 .and. iel.lt.98) then
      ntemp_elastic = 1
      allocate(nedge(ntemp_elastic))
      nedge(:)=0
      coherent_or_incoherent=0
   endif
   if (iel.ge.98) then
      ...
      nedge(:)=0
      last_edge(:)=0
      atomic_z=floor(za/1000)
      atomic_a=mod(nint(za),1000)
      bound_incoherent_xs=0
      bound_coherent_xs=0
      coherent_or_incoherent=0
      ncrystal_msd=0
      leapr_msd=0
      hbar_mine=0.658211951e-3_kr   ! eV*ps
      mass_neutron=1.04540751e-4_kr ! eV*ps^2*Angstrom^-2
      write(nsyso,'(/&
		&  '' NCrystal command ..................... '',(a))')&
		&  ncrystal_command
   endif
   ...
   do while (idone.eq.0)
      ...
      do itemp=1,ntempr
         ...
         !--------------------------------------------------------------------!
         ! Here call the wrapper if iel>98 for each temperature. We also pass !
         ! to the wrapper the current temperature, and for temperatures that  !
         ! that have less bragg edges than the first temperature we pad bragg !
         ! array with the last value (last_edge). For iel 98 and 99, we also  !
         ! check what type of the scatterer the element is, and for iel=99 we !
         ! we scale the appropriate cross section to the sum of the bound     !
         ! coherent and incoherent xs. For coherent scatterers we scale the   !
         ! cross section at each brag, while for the incoherent scatterer we  !
         ! scale the bound incoherent xs by adding the bound coherent_xs to   !
         ! it, which will in endout appropriately scale sb value.             !
         !--------------------------------------------------------------------!
   ...
   if (iel.gt.0 .and. iel.lt.98) then
      ...
      emax=5
      call coher(iel,npr,bragg(:,1),nedge(1),maxb,emax)
   ...
   call endout(ntempr,bragg,nedge,scr,mscr,isym,ilog,bound_incoherent_xs,coherent_or_incoherent,iel)
   ...
   subroutine contin(itemp,np,maxn)
   !--------------------------------------------------------------------
   ...
   do n=2,maxn
      ...
      do j=1,nalpha
         ...
         do k=1,nbeta
            ...
            if ((abs(ssm(k,j,itemp)-zero).gt.epsilon_real).and.n.ge.maxn) then
   ...
   do ialpha=1,nalpha
      ...
      if (abs(ded-zero).lt.epsilon_real) ded=c3*sqrt(twt*al)
      deb=c4*al*deltab
      delta=deb
      ...
      if (nsd.gt.1) then
         ...
         do ibeta=1,nbeta
            ...
            nbt=nsd
            call sbfill(sb,nbt,delta,be,ap,betan,nbeta,ndmax)
            ...
            if (abs(s-zero).gt.epsilon_real) then
               s1=s*exp(-be/2)
               s2=s*exp(-be)
   ...
   !--diffusion branch
   if (abs(c-zero).gt.epsilon_real) then
      if (iprt.eq.1.and.iprint.eq.2) write(nsyso,&
   ...
   subroutine sbfill(sb,nbt,delta,be,s,betan,nbeta,ndmax)
   !--------------------------------------------------------------------
   ...
   do nal=1,nalpha
      ...
      if (iprt.eq.1) then
         ...
         enddo
         if (abs(twt-zero).lt.epsilon_real) sum0=sum0/(1-exp(-al*dwpix(itemp)))
         sum1=sum1/al
         if (iprint.eq.2) then
            write(nsyso,'(''     normalization check ='',f8.4)') sum0
            write(nsyso,'(''          sum rule check ='',f8.4)') sum1
   ...
   if (y.le.one) then
      ...
      do i=1,imax
         bminus(i)=0
         bplus(i)=0
         if (abs(bn(i)-zero).gt.epsilon_real) then
            arg=-dwc-i*betai/2
            bplus(i)=0
   ...
   idone=0
   do while (idone.eq.0)
      if (abs(x-bex(k2)).lt.epsilon_real) then
         sv=sex(k2)
         sint=sv
         return
   ...
   do nal=1,nalpha
      ...
      do i=1,nbeta
         ...
         up=0
         if (abs(exb(i)-zero).gt.epsilon_real) up=ssp(i,nal,itemp)/exb(i)
  ...
  220 continue
   ...
   do i=1,nbeta
      do j=1,nalpha
         ...
         enddo
         if (kk.eq.1) kk=2
         if ((abs(ssm(i,kk-1,itemp)-zero).lt.epsilon_real).or.(abs(ssm(i,kk,itemp)-zero).lt.epsilon_real)) then
            scoh(j)=zero
         else
   ...
   subroutine endout(ntempr,bragg,nedge,scr,mscr,isym,ilog,bound_incoherent_xs,coherent_or_incoherent,iel)
   !--------------------------------------------------------------------
   ! ENDF output routine
   !--------------------------------------------------------------------
   use mainio  ! provide nsyso
   ...
   ! externals
   integer::ntempr,mscr,isym,ilog
   real(kr)::scr(mscr)
   integer::coherent_or_incoherent,iel
   real(kr)::bound_incoherent_xs
   integer,dimension(:),allocatable::nedge
   real(kr),dimension(:,:),allocatable::bragg
   ...
   sb=spr*((1+awr)/awr)**2
   if (abs(aws-zero).gt.epsilon_real) sbs=sps*((1+aws)/aws)**2
   ...
   !--write endf-6 file 1.
   if (iel.eq.0.and.(abs(twt-zero).lt.epsilon_real)) iel=-1
   write(nsyso,'(//)')
   nprnt=0
   nsp=0
   nsc=0
   ...
   if (iel.ne.0) then
      ...
      if (iel.gt.0) ncards=3+(2*nedge(1)+4)/6  
      if (iel.gt.0.and.ntempr.gt.1)&
        ncards=ncards+(ntempr-1)*(1+(nedge(1)+5)/6)
      scr(ii+5)=ncards
      scr(ii+6)=0
      ii=ii+6
   endif
   ...
   !--------------------------------------------------------------------!
   ! Here we also add writting of incoherent elastic part for iel=98 and!
   ! iel=99 when the atom is incoherent scatterer. Incoherent elastic   !
   ! part for iel=100 will be written after the coherent elastic part   !
   !--------------------------------------------------------------------! 
   if ((iel.lt.0).or.(coherent_or_incoherent.eq.2)) then
      mfh=7
      mth=2
      scr(1)=za
      scr(2)=awr
      ...
      call contio(0,nout,nprnt,scr(1),nb,nw)
      !--------------------------------------------------------------------!
      ! Here we assign the SB value (bound incoherent xs)                  !
      !--------------------------------------------------------------------! 
      if (coherent_or_incoherent.eq.2) then
         scr(1)=bound_incoherent_xs
      else
         scr(1)=sb
      endif
      scr(2)=0
   ...
   !--------------------------------------------------------------------!
   ! Here we also add writting of coherent elastic part for iel>=98     !
   !--------------------------------------------------------------------! 
   if ((iel.ge.1) .and. ((coherent_or_incoherent.eq.0) .or. (coherent_or_incoherent.eq.1))) then
      mfh=7
      mth=2
      scr(1)=za
      scr(2)=awr
      !--------------------------------------------------------------------!
      ! Here we set lthr flag to 3 for iel=100                             !
      !--------------------------------------------------------------------! 
      if (iel.lt.100) scr(3)=1
      if (iel.eq.100) scr(3)=3
      scr(4)=0
      scr(5)=0
      scr(6)=0
      call contio(0,nout,nprnt,scr(1),nb,nw)
      ...
      !--------------------------------------------------------------------!
      ! Here we distinguish between iel<98 and iel >= 98. For iel>=98 we   !
      ! do not do any thining out in order to keep all the bragg edges and !
      ! to keep consistent with NCrystal physics. In case the user finds   !
      ! some issues in epithermal region the issue must come from NCrystal.!
      ! NCrystal uses a default parameter for d-spacing cutoff of the bragg!
      ! edges, hence for some materials with big d-spacings it is possible !
      ! that some bragg edges get truncated. This can be fixed by the user !
      ! telling NCrystal to include those values as well. Again, this is a !
      ! not so common occurance.                                           !
      !--------------------------------------------------------------------! 
      sum=0
      suml=0
      w=dwpix(1)
      if (nss.gt.0.and.(abs(b7-zero).lt.epsilon_real)) w=(dwpix(1)+dwp1(1))/2
      do j=1,nedge(1)
         e=bragg(1+2*j-2,1)
         if (iel.gt.0 .and. iel.lt.98) then
            sum=sum+exp(-4*w*e)*bragg(1+2*j-1,1)
         endif
      ...
      enddo
      !--------------------------------------------------------------------!
      ! Because we are not doing any thining out, we need to set jmax equal!
      ! to the number of bragg edges.                                      !
      !--------------------------------------------------------------------!
      if (iel.ge.98) then
         jmax=nedge(1)
      endif
      ...
      do i=1,ntempr
         if (i.eq.1) then
            ...
            sum=0
            w=dwpix(i)
            if (nss.gt.0.and.(abs(b7-zero).lt.epsilon_real)) w=(dwpix(i)+dwp1(i))/2
            j=0
            do while (j.lt.nedge(1))
               j=j+1
               e=bragg(1+2*j-2,1)
               if (j.le.jmax) jj=jj+2
               scr(ii+jj-1-j1)=sigfig(e,7,0)
               !--------------------------------------------------------------------!
               ! Here we distinguish between iel<98 and iel >= 98. The difference   !
               ! between the two is that ncrystal_wrapper passes in energy times    !
               ! the cross section, meaning that the debye-waller factor (i.e. w)   !
               ! has been taken into the account already. This enables the treatment!
               ! of multi-atomic molecules, because the global debye-waller factor  !
               ! for the whole molecule is calculated in NCrystal.                  !
               !--------------------------------------------------------------------! 
               if (iel.gt.0 .and. iel.lt.98) then
                  scr(ii+jj-j1)=sum+exp(-4*w*e)*bragg(1+2*j-1,1)
               else if (iel.ge.98) then
                  scr(ii+jj-j1)=sum+bragg(1+2*j-1,1)
               endif
               sum=scr(ii+jj-j1)
               scr(ii+jj-j1)=sigfig(scr(ii+jj-j1),7,0)
               if (j.ge.nedge(1).or.jj-j1.ge.npage) then
                  if (ii.ge.8) then
                     call tab1io(0,nout,nprnt,scr(1),nb,nw)
                     ii=0
                     j1=npage
            ...
            do j=1,nedge(1)
               ...
               e=sigfig(bragg(1+2*jj-2,1),7,0)
               if (iel.gt.0 .and. iel.lt.98) then
                  scr(ii+jj-j1)=sum+exp(-4*w*e)*bragg(1+2*jj-1,1)
               else if (iel.ge.98) then
                  scr(ii+jj-j1)=sum+bragg(1+2*jj-1,i)
               endif
               sum=scr(ii+jj-j1)
               scr(ii+jj-j1)=sigfig(scr(ii+jj-j1),7,0)
               if (j.ge.nedge(1).or.jj-j1.ge.npage) then
      ...
      enddo
      !--------------------------------------------------------------------!
      ! Here we tell LEAPR to send end of the section command for iel<100  !
      ! else for iel=100 we still need to write the incoherent elastic     !
      !--------------------------------------------------------------------! 
      if (iel.lt.100) call asend(nout,nprnt)
   endif

   !--------------------------------------------------------------------!
   ! Here we add writting of incoherent elastic part for iel=100        !
   !--------------------------------------------------------------------! 
   !--write incoherent elastic part for iel=100
   if (iel.eq.100) then
      mfh=7
      mth=2
      scr(1)=bound_incoherent_xs
